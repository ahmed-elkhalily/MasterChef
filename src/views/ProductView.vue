...
<template>
    <Suspense>
        ...
        <ProductDetails v-else-if="route.params.productId && details" :data="details" />
        <ProductList v-else-if="data" :totalPage="totalPage" :name="name" @changePageHandle=changePageHandle
            @filterHandler="filterHandler" :data="data" />
    </Suspense>
</template>